...
export class RuleVerdictAgent {
  ...
  private async fetchAmendments(ruleId: string): Promise<FetchedAmendment[]> {
    ...
    const reply = await this.runGroq(
      amendmentsForQuery(ruleId, projection),
  ...
  async ask(question: string): Promise<Answer> {
    ...
    const candReply = await this.runGroq(
      candidatesQuery(scenario, candidatesProj),
    ...
    for (const c of candidates) {
      const amds = await this.fetchAmendments(c._id);
    ...
    let resolutionAuthority: FetchedSource | null = null;
