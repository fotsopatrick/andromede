...
- `tests/` — 19 deterministic tests (`node --test`).
...
| R2-bis | several amendments at the **same** maximal precedence target the rule | same value → merged verdict; **contradictory** values → **refusal** (never a silent "not amended" answer) |
...
| R5 | a human `resolutionCase` exists | our formal derivation and the case **must agree**, else the data is anomalous → refusal. A case whose authority reference is **broken** (`_ref` to a missing document) is also an anomaly → refusal |

A temporal guard sits underneath: every query filters `effectiveDate <= today`, so a **future-dated** rule or amendment does not govern yet (and is not silently skipped — see TEST 6e/6f).
...
- `TEST 5 — infra`: GROQ interpreter correctness (types, conditions, ordering on ISO-date strings, **exclusive vs inclusive slices** `[n...m]`/`[n..m]`, rejection of unsupported syntax), schema mirror == Studio schema, seed documents valid against the declared schema.
- `TEST 6 — corners` (the blind spots of the first bank): equal-precedence amendments with contradictory values → refusal (not a silent "not amended"), equal-precedence same-value amendments → merged verdict, `status == "active"` proven on a deprecated rule, broken `resolutionCase` authority reference → anomaly, future-dated amendment *and* rule not applied, the real MCP error path, and `array_field_reader` exercised through the real protocol channel.
