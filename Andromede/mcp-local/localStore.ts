...
export class LocalStore {
  ...
  execute(query: string): GroqExecutionResult {
    ...
    // Parité GROQ réelle : un slice [0] sur un ensemble vide donne null (jamais
    // une clé `result` absente — JSON.stringify n'émet pas les undefined).
    const clean = result === undefined ? null : result;
    return {
      result: clean,
      meta: {
        executedQuery: query,
