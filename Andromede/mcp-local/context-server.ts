        ...
        ? (() => {
            ...
            for (const p of parts) {
              ...
              if (Array.isArray(next)) {
                // Dans le registre, un tableau de définitions n'a qu'un sens de
                // navigation quand il contient UN SEUL item SANS nom propre
                // (ex : of[0] d'un array de types) : on le déroule pour atteindre
                // les champs de l'objet qu'il porte. Un tableau multi-items ou
                // à items nommés (fields) ne se navigue pas via path.
                const first = next[0] as Record<string, unknown> | undefined;
                cursor = next.length === 1 && first?.['name'] === undefined ? first : undefined;
                continue;
              }
              cursor = next;
